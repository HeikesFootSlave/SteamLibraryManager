"""
Game Details Widget - Clean & i18n-ready
...
class GameDetailsWidget(QWidget):
    ...
    def _create_ui(self):
        ...
        # Row 0
        info_grid.addWidget(QLabel(f"<b>{t('ui.game_details.app_id')}:</b>"), 0, 0)
        self.app_id_label = QLabel("")
        info_grid.addWidget(self.app_id_label, 0, 1)

        info_grid.addWidget(QLabel(f"<b>{t('ui.game_details.playtime')}:</b>"), 0, 2)
        self.playtime_label = QLabel("")
        info_grid.addWidget(self.playtime_label, 0, 3)

        # Row 1
        info_grid.addWidget(QLabel(f"<b>{t('ui.game_details.developer')}:</b>"), 1, 0)
        self.developer_label = QLabel("")
        self.developer_label.setWordWrap(True)
        info_grid.addWidget(self.developer_label, 1, 1)

        info_grid.addWidget(QLabel(f"<b>{t('ui.game_details.publisher')}:</b>"), 1, 2)
        ...
        # Edit Button
        edit_btn = QPushButton(t('ui.game_details.edit_metadata_btn'))
    ...
    def set_game(self, game: Game, all_categories: List[str]):
        ...
        playtime = f"{game.playtime_hours}h" if game.playtime_hours > 0 else t('ui.game_details.never_played')
    ...
    def _on_edit(self):
        if self.current_game:
            self.edit_metadata.emit(self.current_game)        info_grid.setColumnStretch(1, 1)
        info_grid.setColumnStretch(3, 1)
