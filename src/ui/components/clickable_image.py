...
class ClickableImage(QWidget):
    ...
    def _create_badges(self, is_animated_file=False):
        ...
        if is_animated_file or 'webp' in mime or 'gif' in mime:
            badges.append({'icon': get_icon_path('flag_animated'), 'fallback': 'GIF', 'color': '#FFD700', 'tip': 'Animated'})
        ...
        if self.metadata.get('nsfw'):
            badges.append({'icon': get_icon_path('flag_nsfw'), 'fallback': '🔞', 'color': '#e74c3c', 'tip': 'Adult Content'})
        if self.metadata.get('epilepsy'):
            badges.append({'icon': get_icon_path('flag_epilepsy'), 'fallback': '⚡', 'color': '#9b59b6', 'tip': 'Epilepsy Warning'})
        if self.metadata.get('lock_tags'):
             badges.append({'icon': get_icon_path('flag_untagged'), 'fallback': '?', 'color': '#95a5a6', 'tip': 'Untagged'})
        ...
        for badge in badges:
            ...
            if badge['icon']:
                ...
                lbl.setStyleSheet(f"background-color: {badge['color']}; color: #000; font-weight: bold; border-radius: 3px; padding: 2px;")
                lbl.adjustSize()
            ...
            try:
                ...
                if is_animated:
                    for frame in ImageSequence.Iterator(im):
                        ...
                        scaled = pix.scaled(self.w, self.h, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
