"""
Metadata Edit Dialogs - Vollständig lokalisiert
"""
...
class MetadataEditDialog(QDialog):
    ...
    def __init__(self, parent, game_name: str, current_metadata: Dict):
        ...
        self.setWindowTitle(f"{t('ui.metadata_editor.title')} - {game_name}")
    ...
    def _create_ui(self):
        ...
        # Title
        title = QLabel(f"{t('ui.metadata_editor.title')}: {self.game_name}")
        title_font = QFont()
        ...
        # Info
        info = QLabel(t('ui.metadata_editor.info_local'))
        ...
        form.addRow(f"{t('ui.metadata_editor.game_name')}:", self.name_edit)
        
        self.sort_as_edit = QLineEdit()
        form.addRow(f"{t('ui.metadata_editor.sort_as')}:", self.sort_as_edit)
        
        sort_help = QLabel(t('ui.metadata_editor.help_sort'))
        sort_help.setStyleSheet("color: gray; font-size: 9px;")
        form.addRow("", sort_help)
        
        self.developer_edit = QLineEdit()
        form.addRow(f"{t('ui.metadata_editor.developer')}:", self.developer_edit)
        
        self.publisher_edit = QLineEdit()
        form.addRow(f"{t('ui.metadata_editor.publisher')}:", self.publisher_edit)
        
        self.release_date_edit = QLineEdit()
        form.addRow(f"{t('ui.metadata_editor.release_date')}:", self.release_date_edit)
        
        date_help = QLabel(t('ui.metadata_editor.help_date'))
        ...
        # Original values
        original_group = QGroupBox(t('ui.metadata_editor.original_values'))
        ...
        reset_btn = QPushButton(t('ui.metadata_editor.reset'))
        reset_btn.clicked.connect(self._reset_to_original)
        btn_layout.addWidget(reset_btn)
        
        cancel_btn = QPushButton(t('ui.metadata_editor.cancel'))
        cancel_btn.clicked.connect(self.reject)
        btn_layout.addWidget(cancel_btn)
        
        save_btn = QPushButton(t('ui.metadata_editor.save'))
    ...
    def _populate_fields(self):
        ...
        original_text = []
        original_text.append(f"{t('ui.metadata_editor.game_name')}: {self.current_metadata.get('name', 'N/A')}")
        original_text.append(f"{t('ui.metadata_editor.developer')}: {self.current_metadata.get('developer', 'N/A')}")
        original_text.append(f"{t('ui.metadata_editor.publisher')}: {self.current_metadata.get('publisher', 'N/A')}")
        original_text.append(f"{t('ui.metadata_editor.release_date')}: {self.current_metadata.get('release_date', 'N/A')}")
    ...
    def _save(self):
        ...
        if not name:
            QMessageBox.warning(self, t('ui.dialogs.errors.invalid_input'), t('ui.metadata_editor.errors.empty_name'))
            return
...
class BulkMetadataEditDialog(QDialog):
    ...
    def _create_ui(self):
        ...
        preview_group = QGroupBox(t('ui.metadata_editor.selected_games', count=self.games_count))
        ...
        if len(self.game_names) > 20:
            preview_names.append(t('ui.metadata_editor.more_games', count=len(self.game_names) - 20))
        ...
        # Checkboxes + Fields
        fields_group = QGroupBox(t('ui.metadata_editor.fields_to_modify'))
        fields_layout = QVBoxLayout()
        
        # Developer
        dev_layout = QHBoxLayout()
        self.cb_developer = QCheckBox(t('ui.metadata_editor.set_developer'))
        self.developer_edit = QLineEdit()
        ...
        self.cb_publisher = QCheckBox(t('ui.metadata_editor.set_publisher'))
        self.publisher_edit = QLineEdit()
        ...
        self.cb_release_date = QCheckBox(t('ui.metadata_editor.set_release_date'))
        self.release_date_edit = QLineEdit()
        ...
        # Warning
        warning = QLabel(t('ui.metadata_editor.bulk_warning'))
        ...
        apply_btn = QPushButton(t('ui.metadata_editor.apply_btn', count=self.games_count))
    ...
    def _apply(self):
        ...
        if not any([
            ...
            QMessageBox.warning(self, t('ui.dialogs.no_changes'), t('ui.dialogs.no_changes'))
            return
        
        # Confirm
        reply = QMessageBox.question(
            self, 
            t('ui.dialogs.bulk_title'),
...
class MetadataRestoreDialog(QDialog):
    ...
    def __init__(self, parent, modified_count: int):
        ...
        self.setWindowTitle(t('ui.metadata_editor.restore_title'))
    ...
    def _create_ui(self):
        ...
        # Title
        title = QLabel(t('ui.metadata_editor.restore_title'))
        title_font = QFont()
        ...
        # Warning
        warning = QLabel(t('ui.dialogs.bulk_warning') if 'bulk_warning' in t('ui.dialogs') else "⚠️ A backup will be created.")
        warning.setStyleSheet("color: orange;")
        layout.addWidget(warning)
        ...
        restore_btn = QPushButton(t('ui.metadata_editor.restore_btn', count=self.modified_count))
    ...
    def should_restore(self) -> bool:
        return self.do_restore        save_btn = QPushButton("Save Changes")
        save_btn.clicked.connect(self._save)
        save_btn.setDefault(True)
        btn_layout.addWidget(save_btn)
